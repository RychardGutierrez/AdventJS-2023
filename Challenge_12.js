...
function checkIsValidCopy(original, copy) {
  ...
  for (let index = 0; index < original.length; index++) {
    ...
    const checkCopy = valid.indexOf(copy[index]);
    const checkOriginal = valid.indexOf(original[index]);
    if (checkCopy < checkOriginal) {
      return false;
    }
  }
  return true;
...
// const result1 = checkIsValidCopy('Santa Clau', 'sa#ta Cl#us'); // true
// const result2 = checkIsValidCopy(
//   's#nta Cla#s is coming',
//   'p#nt: cla#s #s c+min#'
// ); // false (due to the initial p)
const result3 = checkIsValidCopy('Santa Claus', 's#+:. c:. s'); // true
//const result4 = checkIsValidCopy('Santa Claus', 's#+:.#c:. s'); // false (there is a # where it shouldn't be)
console.log(result3);
